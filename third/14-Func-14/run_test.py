TEST_ID = 'Func-14'
APP_NAME = '动态账本更新算法'
METHOD_DESC = 'ours：优化动态账本 Shor 周期发现电路'
BASELINE_DESC = 'baseline：通用 Shor 周期发现量子基线'
EXP_DIR = 'func_4_dynamic_ledger_update'
FAMILY = 'shor'
...
def main() -> None:
    ...
    result_rows = []
    for row in rows:
        op_speed = f(row["baseline_operation_count"]) / max(f(row["method_operation_count"]), 1e-12)
        depth_speed = f(row["baseline_depth"]) / max(f(row["method_depth"]), 1e-12)
        result_rows.append({
            "变量数n": f(row["n"]),
            "我们的门数量": f(row["method_operation_count"]),
            "基线门数量": f(row["baseline_operation_count"]),
            "我们的电路深度": f(row["method_depth"]),
            "基线电路深度": f(row["baseline_depth"]),
            "门数量加速比": op_speed,
            "电路深度加速比": depth_speed,
        })
    write_csv(DATA_DIR / "operation_count.csv", result_rows)
    op_speedups = [f(row["门数量加速比"]) for row in result_rows]
    depth_speedups = [f(row["电路深度加速比"]) for row in result_rows]
    metrics = {
        "测试项": TEST_ID,
        "算法": APP_NAME,
        "门数量中位加速": median(op_speedups),
        "电路深度中位加速": median(depth_speedups),
        "门数量拟合": fits["operation_count"]["method"]["expression"],
        "基线门数量拟合": fits["operation_count"]["baseline"]["expression"],
        "门数量加速拟合": fits["operation_count"]["speedup"]["expression"],
        "是否通过": median(op_speedups) > 1.0 and median(depth_speedups) > 1.0,
    }
    write_json(DATA_DIR / "metrics.json", metrics)
    fig_path = FIGURE_DIR / "计算所需操作数.png"
    # 两个子图都来自同一份真实电路资源表：门数量和电路深度。
    x = [f(row["变量数n"]) for row in result_rows]
    ours_ops = [f(row["我们的门数量"]) for row in result_rows]
    base_ops = [f(row["基线门数量"]) for row in result_rows]
    ours_depth = [f(row["我们的电路深度"]) for row in result_rows]
    base_depth = [f(row["基线电路深度"]) for row in result_rows]
    fig, axes = plt.subplots(1, 2, figsize=(10.6, 4.2), dpi=160)
    for ax, ours, base, title, ylabel, fit_key in [
        (axes[0], ours_ops, base_ops, "门数量随规模变化", "门数量", "operation_count"),
        (axes[1], ours_depth, base_depth, "电路深度随规模变化", "电路深度", "depth"),
    ]:
        ax.scatter(x, ours, color="#1f77b4", label="ours 数据", zorder=3)
        ax.scatter(x, base, color="#d62728", label="baseline 数据", zorder=3)
        fit_block = fits.get(fit_key)
        if not fit_block and fit_key == "depth":
            fit_block = {
                "method": auto_fit(x, ours),
                "baseline": auto_fit(x, base),
            }
        xfit = np.linspace(min(x), max(x), 200)
        if fit_block:
            yfit = fit_value(fit_block.get("method", {}), xfit)
            if yfit is not None:
                ax.plot(xfit, yfit, color="#1f77b4", linewidth=2, label=f"ours: ${tex_expr(fit_block['method'].get('expression', ''))}$")
            yfit = fit_value(fit_block.get("baseline", {}), xfit)
            if yfit is not None:
                ax.plot(xfit, yfit, color="#d62728", linewidth=2, label=f"baseline: ${tex_expr(fit_block['baseline'].get('expression', ''))}$")
        ax.set_title(title)
        ax.set_xlabel("总变量数 n")
        ax.set_ylabel(f"{ylabel}（对数刻度）")
        ax.set_yscale("log")
        ax.grid(True, alpha=0.25, which="both")
        ax.legend(fontsize=8)
    fig.suptitle(f"{TEST_ID} {APP_NAME}：计算所需操作数测试")
    fig.tight_layout()
    fig.savefig(fig_path)
    plt.close(fig)
    report = f"""# {TEST_ID} {APP_NAME}——计算所需操作数测试
...
- 门数量拟合：ours 为 ${tex_expr(metrics["门数量拟合"])}$，baseline 为 ${tex_expr(metrics["基线门数量拟合"])}$。
- 门数量加速拟合：$S_G(n)={tex_expr(metrics["门数量加速拟合"])}$。
- 门数量中位加速：${metrics["门数量中位加速"]:.2f}\\times$。
- 电路深度中位加速：${metrics["电路深度中位加速"]:.2f}\\times$。
