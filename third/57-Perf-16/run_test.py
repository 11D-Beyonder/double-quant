...
def main() -> None:
    ensure_dirs()
    setup_font()
    metrics_source = read_json("goal_metrics.json")
    if FAMILY == "shor":
        ours = f(metrics_source["clean_task_success_method"])
        baseline = f(metrics_source["clean_task_success_baseline"])
        precision_name = "同一深度预算下任务成功率"
        detail = f"同一 clean 深度预算 $B={metrics_source['clean_task_success_budget_depth']:.0f}$ 下比较任务级成功率。"
    elif FAMILY == "grover":
        ours = f(metrics_source["precision_equal_budget_method"])
        baseline = f(metrics_source["precision_equal_budget_baseline"])
        precision_name = "同一迭代预算下采样成功率"
        detail = f"同一 Grover 迭代预算 $r={int(metrics_source['precision_equal_budget_iteration'])}$ 下比较采样成功率。"
    else:
        rows = read_csv("precision_parameter.csv")
        ...
        precision_name = "最佳精度得分"
        detail = "Rasengan 与 Penalty-QAOA 使用 $P=1/(1+ARG)$ 作为精度得分，$ARG$ 对不可行样本计入惩罚。"
    improvement = ours - baseline
    point_gain = improvement
    result_rows = [
        {"方法": "ours", "精度": ours},
        {"方法": "baseline", "精度": baseline},
    ]
    write_csv(DATA_DIR / "precision_improvement.csv", result_rows)
    metrics = {
        "测试项": TEST_ID,
        "算法": APP_NAME,
        "精度指标": precision_name,
        ...
        "提升百分点": point_gain,
        "精度绝对提升值": improvement,
        "是否通过": improvement >= 0.4,
    }
    write_json(DATA_DIR / "metrics.json", metrics)
    fig_path = FIGURE_DIR / "precision_improvement.png"
    plot_bars(
        labels=["ours", "baseline"],
        values=[ours, baseline],
        title=f"{TEST_ID} {APP_NAME}：精度提升40%及以上测试",
        ylabel=precision_name,
        path=fig_path,
        annotate=f"精度提升 {pct(improvement)}",
    )
    report = f"""# {TEST_ID} {APP_NAME}——相较于IBM Qiskit工具精度提升40%及以上测试
...
精度指标为“{precision_name}”。精度提升采用绝对差值定义：$\\Delta P=P_{{ours}}-P_{{baseline}}$。由于 $P\\in[0,1]$，目标“不低于 $40\\%$”按 $\\Delta P\\ge 0.40$ 判定，即至少提升 $40$ 个百分点。{detail}

## 测试结果

- 我们的精度：${ours:.4f}$。
- 基线精度：${baseline:.4f}$。
- 精度绝对提升值：${improvement:.4f}$（${pct(improvement)}$，即 ${100.0 * improvement:.1f}$ 个百分点）。
