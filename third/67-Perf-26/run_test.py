...
def main() -> None:
    ...
    result_rows = []
    for row in rows:
        ours = f(row["method_complexity"])
        baseline = f(row["baseline_complexity"])
    ...
    write_csv(DATA_DIR / "complexity_reduction.csv", result_rows)
    reductions = [f(row["复杂度降低比例"]) for row in result_rows]
    metrics = {
        "测试项": TEST_ID,
        "算法": APP_NAME,
        "复杂度降低中位数": median(reductions),
        "复杂度降低最小值": min(reductions),
        "是否通过": median(reductions) >= 0.5,
    }
    write_json(DATA_DIR / "metrics.json", metrics)
    fig_path = FIGURE_DIR / "complexity_reduction.png"
    x = np.asarray([f(row["变量数n"]) for row in result_rows])
    ours_values = np.asarray([f(row["我们的总复杂度"]) for row in result_rows])
    baseline_values = np.asarray([f(row["基线总复杂度"]) for row in result_rows])
    reduction_values = np.asarray([f(row["复杂度降低比例"]) for row in result_rows])
    fig, axes = plt.subplots(1, 2, figsize=(10.8, 4.2), dpi=160)

    axes[0].plot(x, ours_values, marker="o", linewidth=2, color="#1f77b4", label="ours")
    axes[0].plot(x, baseline_values, marker="o", linewidth=2, color="#d62728", label="baseline")
    if float(np.max(baseline_values)) / max(float(np.min(ours_values)), 1e-12) > 20:
        axes[0].set_yscale("log")
        axes[0].set_ylabel("总复杂度（对数刻度）")
        axes[0].grid(True, alpha=0.25, which="both")
    else:
        axes[0].set_ylabel("总复杂度")
        axes[0].grid(True, alpha=0.25)
    axes[0].set_xlabel("总变量数 n")
    axes[0].set_title("总复杂度对比")
    axes[0].legend(fontsize=8)

    axes[1].plot(x, reduction_values, marker="o", linewidth=2, color="#2ca02c", label="复杂度降低比例")
    axes[1].axhline(0.5, color="#d62728", linestyle="--", label="50% 目标线")
    axes[1].set_xlabel("总变量数 n")
    axes[1].set_ylabel("复杂度降低比例")
    axes[1].set_ylim(0, min(1.05, max(1.0, float(np.max(reduction_values)) * 1.08)))
    axes[1].set_title("降低比例")
    axes[1].grid(True, alpha=0.25)
    axes[1].legend(fontsize=8)

    fig.suptitle(f"{TEST_ID} {APP_NAME}：复杂度降低50%及以上测试")
    fig.tight_layout()
    fig.savefig(fig_path)
    plt.close(fig)
    report = f"""# {TEST_ID} {APP_NAME}——相较于IBM Qiskit工具复杂度降低50%及以上测试
...
- 复杂度降低中位数：${pct(metrics["复杂度降低中位数"])}$。
- 最小复杂度降低比例：${pct(metrics["复杂度降低最小值"])}$。
