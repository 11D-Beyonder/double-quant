...
def plot_shor_space_exact(
    *,
    ...
    x = np.asarray([f(row["变量数n"]) for row in rows], dtype=float)
    ours = np.asarray([f(row["我们的求解空间大小"]) for row in rows], dtype=float)
    baseline = np.asarray([f(row["基线求解空间大小"]) for row in rows], dtype=float)
    order = np.argsort(x)
    x, ours, baseline = x[order], ours[order], baseline[order]
    xfit = np.linspace(float(np.min(x)), float(np.max(x)), 200)
    ours_exact = 2 ** (xfit - 1)
    baseline_exact = 2 ** xfit
    plt.figure(figsize=(7.2, 4.8), dpi=160)
    plt.scatter(x, ours, color="#1f77b4", label="ours 数据", zorder=3)
    plt.scatter(x, baseline, color="#d62728", label="baseline 数据", zorder=3)
    plt.plot(xfit, ours_exact, color="#1f77b4", linewidth=2, label=r"ours: $2^{n-1}$")
    plt.plot(xfit, baseline_exact, color="#d62728", linewidth=2, label=r"baseline: $2^n$")
    plt.xlabel("总变量数 n")
    plt.ylabel(ylabel)
    plt.title(title)
    plt.yscale("log")
...
def main() -> None:
    ...
    metrics = {
        ...
        "说明": "已剔除 n=2 的实现下限点；保留测试点中 baseline/ours 的求解空间倍率恒为 2。",
    }
    write_json(DATA_DIR / "metrics.json", metrics)
    fig_path = FIGURE_DIR / "求解空间大小.png"
    plot_shor_space_exact(
        rows=result_rows,
